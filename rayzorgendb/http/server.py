...
Pakai http.server bawaan Python — tanpa dependensi.
"""
...
class Handler(BaseHTTPRequestHandler):
 ...
 self.db.config.DEFAULT_LIMIT)
 skip = self._int(qs, "skip", 0)
 ...
 # /{collection}/aggregate?field=X&op=sum
 m = re.match(
 r"^/([A-Za-z0-9_-]+)/aggregate$", path
 )
 if m:
 name = m.group(1)
 ...
 return self._json(200, {
 "value": self.db.collection(name)
 .aggregate(field, op)
 })

 # /{collection}/group_by?field=X
 m = re.match(r"^/([A-Za-z0-9_-]+)/group_by$", path)
 if m:
 name = m.group(1)
 field = qs.get("field", [None])[0]
 if not field:
 ...
 op = qs.get("op", ["count"])[0]
 return self._json(200, {
 "groups": self.db.collection(name)
 .group_by(field, agg, op)
 })
 ...
 results = self.db.collection(name).vector_search(
 vec, top_k
 )
...
def run_server(host: str = None, port: int = None,
 config=None, threaded: bool = True):
 ...
 print(f" RayzorgenDB HTTP API: http://{host}:{port}")
 print(f" Coba: curl http://localhost:{port}/health")
 try:
